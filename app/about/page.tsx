...
function getChatterActivities() {
  ...
  const files = fs.readdirSync(chattersDirectory).filter(f => f.endsWith('.md'));
...
function getMomentActivities() {
  ...
  const files = fs.readdirSync(momentsDirectory).filter(f => f.endsWith('.md'));
